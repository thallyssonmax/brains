...
export function AuthForm({locale,recovery=false,onRecovered,callbackError=false}:{locale:Locale;recovery?:boolean;onRecovered:()=>void;callbackError?:boolean}){
 ...
 return <section className="auth-form"><h1>{t[mode]}</h1>{google&&(mode==='login'||mode==='signup')&&<><button className="secondary wide" disabled={busy} onClick={()=>void run(async()=>{const {error}=await cloud!.auth.signInWithOAuth({provider:'google',options:{redirectTo:new URL('/',window.location.origin).href}});if(error)throw error})}>{t.google}</button><p className="small center">{t.or}</p></>}
 ...
 {(error==='unconfirmed'||message==='sent')&&<button className="text-btn" disabled={busy} onClick={()=>void run(async()=>{const {error}=await cloud!.auth.resend({type:'signup',email:email.trim(),options:{emailRedirectTo:new URL('/',window.location.origin).href}});if(error)throw error;setMessage('sent')})}>{t.resend}</button>}
