...
describe('daily-only scheduling',()=>{
 ...
 it('does not revive a legacy intraday card already reviewed today',()=>{const db=fixture();const c=reviewQueue(db,now)[0];applyReview(db,{id:'legacy',cardId:c.id,at:now.toISOString(),known:false});c.memory!.due=new Date(now.getTime()+60000);expect(reviewQueue(db,new Date(now.getTime()+120000)).some(x=>x.id===c.id)).toBe(false)});
});
